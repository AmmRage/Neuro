...
namespace AForge.Controls
{
	...
	public class Chart : Control
	{
		...
		{
			var	g = pe.Graphics;
			var			clientWidth = this.ClientRectangle.Width;
			var			clientHeight = this.ClientRectangle.Height;

			// fill with white background
			g.FillRectangle(this.whiteBrush, 0, 0, clientWidth - 1, clientHeight - 1 );
			...
			if (this.rangeY != null )
			{
				var xFactor = (double)( clientWidth - 10 ) / (this.rangeX.Length );
				var yFactor = (double)( clientHeight - 10 ) / (this.rangeY.Length );

				// walk through all data series
				var en = this.seriesTable.GetEnumerator( );
				while ( en.MoveNext( ) )
				{
					...
					if ( series.type == SeriesType.Dots )
					{
						...
						{
							var x = (int) ( ( data[i, 0] - this.rangeX.Min ) * xFactor );
							var y = (int) ( ( data[i, 1] - this.rangeY.Min ) * yFactor );
					...
					else if ( series.type == SeriesType.ConnectedDots )
					{
						...
						var x1 = (int) ( ( data[0, 0] - this.rangeX.Min ) * xFactor );
						var y1 = (int) ( ( data[0, 1] - this.rangeY.Min ) * yFactor );
						...
						{
							var x2 = (int) ( ( data[i, 0] - this.rangeX.Min ) * xFactor );
							var y2 = (int) ( ( data[i, 1] - this.rangeY.Min ) * yFactor );
		...
		{
			// get data series
			var	series = (DataSeries) this.seriesTable[name];
			// update data
			series.data = data;
		...
		private void UpdateYRange( )
		{
			...
			{
			    this.rangeY = new DoubleRange( minY, maxY );
			}
		}
	}
}
