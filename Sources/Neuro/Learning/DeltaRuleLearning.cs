...
namespace AForge.Neuro.Learning
{
	...
	public class DeltaRuleLearning : ISupervisedLearning
	{
		...
		public double Run( double[] input, double[] output )
		{
			...
			var networkOutput = this.network.Compute( input );

			// get the only layer of the network
			var layer = this.network[0];
			...
			for ( int j = 0, k = layer.NeuronsCount; j < k; j++ )
			{
				...
				{
					neuron[i] += this.learningRate * e * functionDerivative * input[i];
				}

				// update threshold value
				neuron.Threshold += this.learningRate * e * functionDerivative;
